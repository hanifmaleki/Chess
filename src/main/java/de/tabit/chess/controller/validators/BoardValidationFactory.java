...
/**
 * Created by Hanif Maleki on 5/11/18. This class is a factory class for generating validation rules
 * of the loading board. It generates the predicates which are responsible for checking specific
...
public class BoardValidationFactory {
  ...
  public static Map<Predicate<BoardStatus>, String> getPredicates() {
    ...
    Predicate<BoardStatus> allPiecesLimitation =
        boardStatus -> boardStatus.getCurrentPieces().size() <= 32;
    predicateStringMap.put(allPiecesLimitation, "Number of pieces exceeded.");

    for (Side side : Side.values()) {
      Predicate<BoardStatus> kingLimitation =
          boardStatus -> boardStatus.getNumberOf(PieceFactory.getKing(side)) == 1;
      predicateStringMap.put(kingLimitation, "number of " + side.name() + " king is incorrect.");

      Predicate<BoardStatus> pawnLimitation =
          boardStatus -> boardStatus.getNumberOf(PieceFactory.getPawn(side)) <= 8;
      predicateStringMap.put(pawnLimitation, "number of " + side.name() + " pawn is exceeded.");

      Predicate<BoardStatus> knightLimitation =
          boardStatus ->
      ...
      Predicate<BoardStatus> bishopLimitation =
          boardStatus ->
              boardStatus.getNumberOf(PieceFactory.getBishop(side)) <= TOTAL_ALLOWED_PIECE_NUMBER;
      predicateStringMap.put(
          bishopLimitation, "number of " + side.name() + " bishops is exceeded.");

      Predicate<BoardStatus> castleLimitation =
          boardStatus ->
              boardStatus.getNumberOf(PieceFactory.getCastle(side)) <= TOTAL_ALLOWED_PIECE_NUMBER;
      predicateStringMap.put(
          castleLimitation, "number of " + side.name() + " castles is exceeded.");

      Predicate<BoardStatus> queenLimitation =
          boardStatus ->
